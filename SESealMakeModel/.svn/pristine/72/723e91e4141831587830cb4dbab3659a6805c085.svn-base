;(function() {
  ...
  function roundSeal(id, option) {
    ...
    // 边框线的宽度
    this.lineWidth =
      this.mmConversionPx(option.lineWidth) || this.mmConversionPx(1.2)
    // alert(this.lineWidth)
    //是否有内边线
    this.hasInnerLine = option.hasInnerLine || false
    // 内边框线的宽度
    this.innerLineWidth =
      this.mmConversionPx(option.innerLineWidth) || this.mmConversionPx(0.5)
    // 机构名称
    ...
    this.groupNameFontSize =
      this.mmConversionPx(option.groupNameFontSize) || this.mmConversionPx(7)
    // 字体
    ...
    // 字与边框的距离
    this.lineTextGap =
      this.mmConversionPx(option.lineTextGap) || this.mmConversionPx(0.8)
    //文字间距
    this.companyFontWidth =
      this.mmConversionPx(option.companyFontWidth) || this.mmConversionPx(2)
    //防伪码
    this.securityCodeFontSize =
      this.mmConversionPx(option.securityCodeFontSize) || this.mmConversionPx(9)
    //防伪码字宽
    this.securityCodeWidth =
      this.mmConversionPx(option.securityCodeWidth) || this.mmConversionPx(10)
    this.securityCode = option.securityCode || ''
    //印章名称
    this.drawSealName = option.drawSealName || ''
    this.drawSealFontSize =
      this.mmConversionPx(option.drawSealFontSize) || this.mmConversionPx(6)
    //印章名称位置
    this.drawSealLocation = this.mmConversionPx(option.drawSealLocation) || ''
    //印章名称文字间距
    this.drawSealMaxWidth =
      this.mmConversionPx(option.drawSealMaxWidth) || this.mmConversionPx(20)
    this.innerLineDistance =
      this.mmConversionPx(option.innerLineDistance) || this.mmConversionPx(8)
    //五角星
    this.starX = option.starX || 13
    this.starY = 2 * option.radius
    this.fiveStar = (this.starX / this.starY) * this.radius
    this.fiveStarX =
      this.mmConversionPx(option.fiveStarX) || this.mmConversionPx(0)
    // 初始化
    this._init()
  }
  ...
  //印章文字判断
  roundSeal.prototype._drawText = function _drawText(
    text,
    fontSize,
    isTypeName,
    isSecurityCode
    ...
    if (isTypeName) {
      ...
      this.ctx.fillText(
        text,
        0,
        this.drawSealLocation + this.drawSealFontSize / 2,
        this.drawSealMaxWidth
      )
    } else {
      //公司，防伪码
  ...
  //文字实现方式
  roundSeal.prototype._drawLetter = function _drawLetter(
    letter,
    angle,
    y,
    maxwidth,
